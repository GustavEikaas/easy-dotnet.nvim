...
local function get_or_set_cached_value(project_file_path, target_framework)
  local co = coroutine.running()
  if not co then error("Function has to run as coroutine") end
  M.preload_msbuild_properties(project_file_path, function(properties) coroutine.resume(co, properties) end, target_framework)
...
M.get_project_from_project_file = function(project_file_path)
  local result = file_cache.get(project_file_path, function(lines)
    local msbuild_props = get_or_set_cached_value(project_file_path)
    ...
    project.get_specific_runtime_definition = function(target_framework)
      ...
      local msbuild_target_framework_props = get_or_set_cached_value(project_file_path, target_framework)
