...
function M.extract(vars, cb)
  for _, entry in ipairs(vars) do
    if entry.name == "list" and entry.variablesReference and entry.variablesReference ~= 0 then
      require("easy-dotnet.netcoredbg").fetch_variables(entry.variablesReference, 1, function(children) require("easy-dotnet.netcoredbg.value_converters.list").extract(children, cb) end)
      return
    end
  end
end
