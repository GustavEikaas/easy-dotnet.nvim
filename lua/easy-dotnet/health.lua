...
local function check_debugger_configured()
  ...
  if auto_register == false then
    vim.health.warn("auto_register_dap config is disabled, you're on your own")
    return
  end
  ...
  for key, _ in pairs(s.adapters) do
    if key == constants.debug_adapter_name then
      vim.health.ok("debugger is configured")
      return
    end
  end
  vim.health.error("Unable to find auto_registered_dap_config, is it being overwritten?", { "https://github.com/GustavEikaas/easy-dotnet.nvim?tab=readme-ov-file#nvim-dap-configuration" })
end
