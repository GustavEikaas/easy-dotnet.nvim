  ...
  for _, value in ipairs(M.lines) do
    if value.hidden == false or value.hidden == nil then
      ...
      if value.icon == M.options.icons.failed then
        vim.api.nvim_buf_add_highlight(M.buf, ns_id, "DiagnosticError", shadow_index - 1, 0, -1)
      elseif value.icon == "<Running>" then
        vim.api.nvim_buf_add_highlight(M.buf, ns_id, "DiagnosticHint", shadow_index - 1, 0, -1)
      elseif value.icon == M.options.icons.passed then
        vim.api.nvim_buf_add_highlight(M.buf, ns_id, "DiagnosticOk", shadow_index - 1, 0, -1)
