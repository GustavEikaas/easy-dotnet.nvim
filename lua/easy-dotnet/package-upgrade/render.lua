...
-- line index → packageId (1-based, set during render)
local line_to_pkg = {}

local ICON_CHECKED = "󰄲" -- nf-md-checkbox_marked
local ICON_UNCHECKED = "󰄱" -- nf-md-checkbox_blank_outline
...
local _writing = false -- re-entrancy guard
...
--- Returns the candidate under the cursor, or nil.
...
--- Rebuild the buffer contents from current state.
function M.refresh()
  ...
  -- ── Compute column widths from visible candidates ─────────────────────────
  ...
  local col_ver = 8
  for _, c in ipairs(state.candidates) do
    local is_major = c.upgradeSeverity == "Major"
    if is_major and state.mode == "safe" then goto skip end
  ...
  -- ── Header — left status + right severity counts (test-runner style) ──────
  ...
  -- Count all candidates by severity (regardless of current mode filter)
  local n_major, n_minor, n_patch = 0, 0, 0
  for _, c in ipairs(state.candidates) do
    if c.upgradeSeverity == "Major" then
      n_major = n_major + 1
    elseif c.upgradeSeverity == "Minor" then
      n_minor = n_minor + 1
    else
      n_patch = n_patch + 1
    end
  ...
  if phase == "Applying" and state.progress then
    ...
    left = string.format("  󰏖  NuGet Upgrade Wizard    %s%s  [%s] %3d%%  %s", spin_icon, phase, bar, pct, p.packageId)
  else
    left = string.format("  󰏖  NuGet Upgrade Wizard    mode: %s    %s%s    %d/%d selected", mode_label, spin_icon, phase, selected_n, count)
  end

  -- Right side: result summary when available, otherwise severity counts
  local win_width = (M.win and vim.api.nvim_win_is_valid(M.win)) and vim.api.nvim_win_get_width(M.win) or 90

  local right_parts = {} -- { text, hl }
  if state.result then
    ...
    if err_n > 0 then table.insert(right_parts, { string.format("  󰅖 %d failed", err_n), "DiagnosticError" }) end
  ...
  -- Build per-segment highlights for the header row
  local header_row = #lines -- 0-based after insert
  table.insert(lines, header_line)
  -- Left side highlight spans the full left text (byte length, not display width)
  table.insert(hls, { header_row, 0, #left, phase_hl[phase] or "Normal" })
  -- Right side per-segment highlights (byte offsets)
  local byte_col = #left + pad
  for _, part in ipairs(right_parts) do
    if part[2] then table.insert(hls, { header_row, byte_col, byte_col + #part[1], part[2] }) end
  ...
  -- ── Column headings ───────────────────────────────────────────────────────
  if count > 0 then
    local heading = string.format("     %s  %s  %s  %s", rpad("Package", col_id), rpad("Current", col_ver), rpad("Target", col_ver), "Severity")
    set_line(lines, hls, heading, "Comment")
    set_line(lines, hls, string.rep("─", 72), "Comment")
  end

  -- ── Package list ─────────────────────────────────────────────────────────
  -- Build a fast lookup for failed packages from the last result
  local failed_map = {} -- packageId → error string
  if state.result then
  ...
  elseif count == 0 then
    ...
    for _, c in ipairs(state.candidates) do
      local is_major = c.upgradeSeverity == "Major"
      if is_major and state.mode == "safe" then goto continue end

      local failed_err = failed_map[c.packageId]
      local icon, row_hl
      if failed_err then
        icon = "󰅖" -- nf-md-close_circle
      ...
      local line_text = string.format("  %s  %s  %s  %s  %s%s%s%s", icon, rpad(c.packageId, col_id), rpad(c.currentVersion, col_ver), rpad(target, col_ver), c.upgradeSeverity, cpm, pin_hint, err_hint)

      local lnum = set_line(lines, hls, line_text, row_hl)
      line_to_pkg[lnum] = c.packageId

      -- Highlight the pinned target version in yellow so it stands out
      if override then
        local target_col_start = 2 + 4 + col_id + 2 + col_ver + 2 -- icon(4) + spaces
        -- approximate byte offset: "  " + icon(4bytes) + "  " + id_col + "  " + ver_col + "  "
        local byte_prefix = 2 + 4 + 2 + col_id + 2 + col_ver + 2
        local byte_end = byte_prefix + #target
        vim.api.nvim_buf_add_highlight(M.buf, ns_id, "DiagnosticWarn", lnum - 1, byte_prefix, byte_end)
      end
  ...
  -- ── Error pane (restore errors for a failed package) ─────────────────────
  ...
  if focused then
    ...
    if cl == nil or (cl.source == "none" and cl.body == nil) then
      ...
      if loading then
        ...
        if cl and cl.nugetUrl and cl.nugetUrl ~= "" then set_line(lines, hls, "       " .. cl.nugetUrl, "Underlined") end
      end
    else
  ...
  set_line(lines, hls, "", nil)
  set_line(lines, hls, string.rep("─", 72), "Comment")
  set_line(lines, hls, "  <Space> toggle  a safe-all  A all  X clear  u apply  v pin-ver  K changelog  e errors  m mode  q quit", "Comment")
  ...
  for _, hl in ipairs(hls) do
    ...
    if M.buf and vim.api.nvim_buf_is_valid(M.buf) then vim.api.nvim_buf_add_highlight(M.buf, ns_id, group, row0, cs, ce) end
  end
...
--- Open the wizard floating window.
function M.open()
