...
function M.refresh()
  ...
  if phase == "Applying" and state.progress then
    ...
    left = string.format("  󰏖  NuGet Upgrade Wizard    %s%s  [%s] %3d%%  %s",
      spin_icon, phase, bar, pct, p.packageId)
  else
  ...
  if state.focused_error_pkg then
    ...
    set_line(lines, hls, string.rep("─", 72), "Comment")
    set_line(lines, hls,
      string.format("  󰅖  Restore error: %s", state.focused_error_pkg), "DiagnosticError")
    set_line(lines, hls, "", nil)
    if err_msg then
  ...
  for _, hl in ipairs(hls) do
    ...
    if M.buf and vim.api.nvim_buf_is_valid(M.buf) then
      vim.api.nvim_buf_add_highlight(M.buf, ns_id, group, row0, cs, ce)
    end
  end
